...
- Keep private agent session URLs and local user paths out of commits, files, release notes, logs, and generated artifacts. Contributors choose the email address published with their commits. Agent attribution and co-author trailers are allowed when they do not expose private session details.
- Run `npm run verify:privacy` before every commit, push, tag, or release. Do not bypass the versioned Git hooks with `--no-verify`. Fix a blocked value at its source and create a clean commit.
- Publish only reviewed commits based on the remote branch tip. Never push private local branch history or rewrite published history to work around a privacy failure.
